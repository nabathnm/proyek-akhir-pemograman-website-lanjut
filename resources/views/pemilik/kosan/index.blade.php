    ...
    <x-slot name="header">
        <div class="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
            <div>
                ...
                <h2 class="text-4xl font-black leading-none">Kosan Saya</h2>
            </div>
            <a href="{{ route('pemilik.kosan.create') }}" class="nb-btn nb-btn-primary">Tambah Kosan</a>
        </div>
    </x-slot>
        ...
        <div class="nb-shell">
            @if($kosans->isEmpty())
                <section class="nb-card p-10 text-center">
                    ...
                    <p class="mt-2 text-lg font-medium">Tambahkan kosan pertama Anda sekarang.</p>
                    ...
                    @foreach($kosans as $kosan)
                        <article class="nb-card overflow-hidden">
                            ...
                            <div class="p-4">
                                ...
                                <p class="mt-1 text-2xl font-black">Rp {{ number_format($kosan->harga_per_bulan, 0, ',', '.') }}</p>
                                <p class="nb-kicker mt-1">{{ $kosan->kamar_tersedia }}/{{ $kosan->jumlah_kamar }} kamar</p>

                                <div class="mt-4 grid grid-cols-3 gap-2">
                                    <a href="{{ route('pemilik.kosan.show', $kosan) }}" class="nb-btn py-2 text-base">Detail</a>
                                    <a href="{{ route('pemilik.kosan.edit', $kosan) }}" class="nb-btn nb-btn-secondary py-2 text-base">Edit</a>
                                    <form action="{{ route('pemilik.kosan.destroy', $kosan) }}" method="POST" onsubmit="return confirm('Hapus kosan ini?')">
                                        @csrf
                                        @method('DELETE')
