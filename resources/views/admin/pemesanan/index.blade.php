<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center gap-2">
            ...
            <h2 class="text-4xl font-black leading-none">Semua Pemesanan</h2>
        </div>
    </x-slot>
        ...
        <div class="nb-shell space-y-6">
                        ...
                        <article class="nb-card p-4 md:p-5">
                            <div class="grid gap-4 md:grid-cols-[160px_minmax(0,1fr)_auto] md:items-start">
                                ...
                                <div class="flex min-w-[180px] flex-col gap-2 md:items-stretch">
                                    <a href="{{ route('admin.pemesanan.show', $pemesanan) }}" class="nb-btn w-full py-2 text-base">Detail</a>
                                    @if($pemesanan->status === 'pending')
                                        <form method="POST" action="{{ route('admin.pemesanan.setujui', $pemesanan) }}">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="nb-btn nb-btn-primary w-full py-2 text-base">Setujui</button>
                                        </form>
                                        <form method="POST" action="{{ route('admin.pemesanan.tolak', $pemesanan) }}" onsubmit="return confirm('Yakin ingin menolak pemesanan ini?')">
                                            @csrf
                                            @method('PATCH')
            ...
            @else
                <section class="nb-card p-10 text-center">
                    ...
                    <p class="mt-3 text-lg font-medium">Belum ada pengajuan yang masuk.</p>
                </section>
            @endif
        </div>
    </div>
